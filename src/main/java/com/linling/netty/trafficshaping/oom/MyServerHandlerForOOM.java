 ...
 * 而数据的写出速率有被限制住了（对端接收带宽限制为了 1 M/s，这样当对端的接收缓冲区满的话，TCP的滑动窗口就会变为0，这样本段也就不会继续发送数据了 ），导致待发送的数据会积压在ChannelOutboundBuffer中，然后新的write操作又会不断从池中申请内存（旧的数据一致未被写出去，内存一直被占用的），最终导致OOM。注意，这里申请的是对外内存。
